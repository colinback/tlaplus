...
VARIABLE votes,   \* votes[a] is the set of votes cast by acceptor a
         maxBallot   \* maxBallot[a] is a ballot number.  Acceptor a will cast
                  \*   further votes only in ballots numbered \geq maxBallot[a]

(***************************************************************************)
(* The type-correctness invariant.                                         *)
(***************************************************************************)
TypeOK == /\ votes \in [Acceptor -> SUBSET (Ballot \X Value)]
          /\ maxBallot \in [Acceptor -> Ballot \cup {-1}]
-----------------------------------------------------------------------------
(***************************************************************************)
...
CannotVoteAt(a, b) == /\ maxBallot[a] > b
                      /\ DidNotVoteAt(a, b)
  (*************************************************************************)
  (* Because acceptor a will not cast any more votes in a ballot numbered  *)
  (* < maxBallot[a], this implies that a has not and will never cast a vote   *)
  (* in ballot b.                                                          *)
  (*************************************************************************)
...
(*                                                                         *)
(* An acceptor a is allowed to increase maxBallot[a] to a ballot number b at  *)
(* any time.                                                               *)
(***************************************************************************)
IncreasemaxBallot(a, b) ==
  /\ b > maxBallot[a]
...
(* maxBallot[a].  The next two conjuncts maintain the invariance of           *)
...
Spec == Init /\ [][Next]_<<votes, maxBallot>>
...
THEOREM Spec => C!Spec 
<1>1. Inv /\ Init => C!Init
<1>2. Inv /\ [Next]_<<votes, maxBallot>> => [C!Next]_chosen
<1>3. QED
  <2>1. []Inv /\ [][Next]_<<votes, maxBallot>> => [][C!Next]_chosen
